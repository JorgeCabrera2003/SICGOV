...
class BusinessSeeder
{
    ...
    private function crearPersonasYUsuarios()
    {
        ...
        if (!$personaExists) {
            ...
            $sqlEmpleado = "INSERT INTO empleado (cedula, id_cargo, fecha_ingreso) 
                            VALUES ('V-00000000', 'CARGO00520260519200547232', :fecha_ingreso)";
            $stmt = $this->db->prepare($sqlEmpleado);
            $stmt->execute(['fecha_ingreso' => date('Y-m-d')]);

            echo "       Persona y Empleado (Gerente) para Admin Root creados.\n";
        }
